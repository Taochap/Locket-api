...
def spam_friend_request():
    with z_locket_instance.command_lock:
        if z_locket_instance.command_in_use and time.time() < z_locket_instance.command_expiry_time:
            remaining_time = int(z_locket_instance.command_expiry_time - time.time())
            return jsonify({
                'status': 'error',
                'message': f'Server đang bận. Vui lòng thử lại sau {remaining_time // 60} phút {remaining_time % 60} giây.',
                'command_status': '1/1'
            }), 429
        else:
            z_locket_instance.command_in_use = True
            # Sử dụng thời gian còn lại của proxy làm thời gian chờ lệnh
            proxy_time_remaining = z_locket_instance.get_proxy_time_remaining()
            # Nếu thời gian còn lại ít hơn 1 phút, đặt tối thiểu 1 phút
            command_wait_time = max(proxy_time_remaining, 60)
            z_locket_instance.command_expiry_time = time.time() + command_wait_time
    ...
    if not target_url:
        with z_locket_instance.command_lock:
            z_locket_instance.command_in_use = False
...
def command_status():
    with z_locket_instance.command_lock:
        in_use = z_locket_instance.command_in_use
        expiry_time = z_locket_instance.command_expiry_time

    remaining_time = 0
    if in_use and time.time() < expiry_time:
        remaining_time = int(expiry_time - time.time())

    status_text = '1/1' if in_use and remaining_time > 0 else '0/1'
